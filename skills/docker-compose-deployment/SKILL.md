...
curl -s http://localhost:<port>/api/v1/vehicles/
```

## Common Pitfalls

### FastAPI trailing-slash redirect (307)
- `GET /api/v1/vehicles` → **307 Temporary Redirect** to `/api/v1/vehicles/`
...
- **Fix:** Use the container name + internal port from within the container:
  ```python
  BASE = "http://mvp_output-api-1:8000/api/v1"
  ```
...
- **Root cause:** Endpoint paths in test scripts don't match the actual router definitions. Router prefixes are `garage/` not `service-records/` or `service-bookings/`.
- **Fix — discover paths from source:**
  ```python
  # Read router files to find actual prefixes and paths
  # service_records.py: prefix = "/garage/records"
  # service_bookings.py: prefix = "/garage/bookings"
  # Correct paths: /api/v1/garage/records/, /api/v1/garage/bookings/
  ```
...
- **Root cause:** Compose files (`docker-compose.yml`, `docker-compose.uat.yml`) live in a subdirectory (e.g., `mvp_output/`), not the project root.
...
### Vehicle image naming convention mismatch
- **Symptom:** Seed `image_url` paths (`byd_atto3.png`) don't match actual files (`byd-atto3-blue.png`). Zero matches between seed data and filesystem.
- **Root cause:** Old seed data uses `byd_{model}.png` (underscore, no color). New files use `byd-{model}-{color}.{ext}` (hyphen, with color).
- **Fix — reconcile:**
  1. List actual files: `os.listdir("app/static/vehicles/")`
  2. Build model-to-file mapping dict
  3. Update all `image_url` values in seed JSON
  4. Set `null` for vehicles without images
  5. Add new seed entries for models that have images but no DB records
  6. Verify every `image_url` references an existing file
- **Naming rule:** `brand-model-{variant}-{color}.{ext}` — ensures files are discoverable, sortable, and consistent with the vehicle they depict.
...
- **Google Drive source:** Copy via `shutil.copy()` (not `copy2`) to avoid xattr errors. Check subdirectories (`electric/`, `phev/`) if the file isn't at the root.
- **Reference:** See `references/vehicle-image-naming-convention.md`
...
- **Symptom:** Page renders with broken or missing styles, or new UI components (e.g., membership card) don't appear. `curl http://localhost:<port>/static/css/style.css` returns an older/smaller version. `wc -c` on the host vs container shows a byte-count mismatch.
- **Root cause:** Files modified on the host (CSS, JS, Jinja2 templates) are invisible inside the container until the Docker image is rebuilt. The `COPY . .` in the Dockerfile captured the old version. `docker compose restart` alone does NOT update the file — only `docker compose build` does.
- **Diagnose:**
  ```bash
  # Check file sizes
  wc -c path/on/host/file.ext
  ### Static asset staleness (CSS/JS/Jinja2 template content changes)
  ...
  - **Fast path — `docker cp` hot-deploy:**
    ...
    docker cp app/templates/profile.html mvp_output-api-1:/app/app/templates/profile.html

    # Python code (routers, schemas, services) — requires restart
    docker cp app/routers/customers.py mvp_output-api-1:/app/app/routers/customers.py
    docker compose restart api

    # Static assets (CSS/JS) — NO restart needed
    docker cp app/static/css/style.css mvp_output-api-1:/app/app/static/css/style.css
    ```
...
docker run -d --name <container_name> -p 8000:8000 \
  -e DATABASE_URL="sqlite+aiosqlite:///./gloryev.db" \
...
- **Bulk fix strategy:** Use `execute_code` to search+replace all `user_id` → `customer_id` in `garage_service.py` and related files.
...
- **Fix — direct SQL migration script:**
  ...
  async def create_tables():
      ...
      async with engine.begin() as conn:
          ...
          await conn.execute(text("""
              INSERT INTO hiring_plans (id, name, ...)
...
- **PostgreSQL UUID format requirement:** Table `id` columns with UUID type reject non-UUID strings. Values like `'plan-trial'` cause `InvalidTextRepresentationError: invalid input syntax for type uuid`.
  **Fix — use valid UUID format:**
  ...
  INSERT INTO hiring_plans (id, name, ...)
  VALUES ('a1000000-0000-0000-0000-000000000001', 'Trial Hire', ...),
         ('a1000000-0000-0000-0000-000000000002', 'Short Term', ...);
  ```
...
- **Symptom:** Seed script fails with `UndefinedColumnError` on INSERT — e.g., `column "mileage" of relation "service_records" does not exist`.
...
- **Common mismatches:**
  - `service_records`: no `created_at`, no `mileage` — uses `date`, `description`, `cost`, `status`, `notes`
  - `service_feedback`: no `rating` — uses `overall_rating`, `quality_rating`, `staff_rating`, `value_rating` (4 separate integer columns, 1-5 range)
...
- **Symptom:** `get_or_create_membership` or similar auto-create logic works at runtime, but `alembic/versions/001_initial.py` does not define the table. Fresh DB provision fails.
...
- **Seeding auto-created tables:** When manually `INSERT`-ing into an auto-created table, include **ALL NOT NULL columns**. Discover required columns with:
  ...
  ```
  Include `tier_since`, `status`, `created_at`, `updated_at` for membership tables. Use `ON CONFLICT (user_id) DO NOTHING` for idempotent seeding.

### Seed script: FK cascade deletion order
- **Symptom:** Seed script fails with `IntegrityError: foreign key constraint — <table>` when trying to DELETE from parent tables (e.g., `users`, `vehicles`).
- **Root cause:** Newer tables (memberships, service_bookings, service_records, service_feedback, invoices, service_reminders, service_galleries, membership_rewards) have FKs to `users` or `vehicles` but are NOT in the seed script's deletion list.
- **Fix:** Query the DB for all FK dependencies, then delete in reverse topological order (leaf → root).
  ...
  # Repeat for vehicles, orders, configurations, service_records, service_bookings
  ```
  **Correct deletion order (most dependent first):**
  service_galleries → service_feedback → invoices → service_reminders → membership_rewards → service_records → service_bookings → memberships → payments → order_items → documents → bookings → orders → wishlists → configurations → vehicles → users
...
- **Recommendation:** Prefer **flat directory structures** (`/app/static/vehicles/`) over nested — more flexible for multi-brand support (e.g., `vehicles/byd-*`, `vehicles/tesla-*`, `vehicles/xiaomi-*` all at one level). When `docker cp` creates nesting, flatten immediately and update seed paths accordingly.
- **Seed path update:** After flattening, update seed data `image_url` from `vehicles/vehicles/byd-*.{ext}` → `vehicles/byd-*.{ext}` (flat).
...
- **Symptom:** `service_records` ↔ `invoices` have mutual FK references. No deletion order satisfies both constraints simultaneously.
- **Root cause:** Circular foreign keys between two tables (e.g., `service_records.invoice_id → invoices` AND `invoices.service_record_id → service_records`).
- **Fix — temporarily drop one constraint:**
  ```sql
  ALTER TABLE service_records DROP CONSTRAINT service_records_invoice_id_fkey;
  -- Now delete in any order
  DELETE FROM service_records WHERE vehicle_id = '<id>';
  DELETE FROM invoices WHERE vehicle_id = '<id>';
  ALTER TABLE service_records ADD CONSTRAINT service_records_invoice_id_fkey FOREIGN KEY (invoice_id) REFERENCES invoices(id) ON DELETE CASCADE;
  ```
...
- **Fix — remove or align `response_model`:**
  ...
  async def list_vehicles(...):
      ...
      return {"items": vehicles, "total": total, ...}  # dict, not list!
...
- **Cascading cleanup:** When removing a column from the model, update ALL references:
  - Router response dicts (e.g., `plate_number` → removed from `list_hireable_vehicles`)
  - Test fixtures (e.g., `plate_number=f"GLR-..."` → removed from `conftest.py`)
  - Seed scripts (e.g., `plate_number=...` → removed from INSERT)
  - `__repr__` method
...
- **Root cause:** Router returns paginated envelope dict, but JS does `const vehicles = await resp.json()` and iterates `vehicles.forEach()`.
...
- **Pagination controls:**
  ...
  pagination.innerHTML = `
    <button ${!data.has_prev ? 'disabled' : ''} onclick="loadVehicles(${data.page - 1})">← Prev</button>
    <span style="padding:8px">Page ${data.page}</span>
    <button ${!data.has_next ? 'disabled' : ''} onclick="loadVehicles(${data.page + 1})">Next →</button>
  `;
  ```
...
curl -s http://localhost:8010/api/v1/hire/vehicles/
...
# 6. Verify fix
sleep 5 && curl -s http://localhost:8010/api/v1/hire/vehicles/
...
- **Reference:** See `references/gloryev-seed-fk-cascade.md` for FK cascade deletion order, `references/async-seed-script-debugging.md` for async seed script timeout debugging, and `references/garage-page-deployment-debug.md` for CSS staleness.
...
- **Recovering files after destructive git operation:**
  ...
  - **Recovery:** Copy files from the running container back to the host:
    ...
    docker cp <container>:/app/path/to/file.py ./mvp_output/path/to/file.py
    ```
  - **Batch recovery:** Use `scripts/recover_from_container.sh`:
    ```bash
    ./scripts/recover_from_container.sh mvp_output-api-1 ./mvp_output \
      app/models/hiring.py app/routers/hiring.py app/main.py ...
    ```
...
- **Enum creation order:** PostgreSQL enum types referenced in ORM models (e.g., `hiringplantype`, `hiringstatus`) must be created BEFORE `metadata.create_all()` is called. Create them via raw SQL in a separate session:
  ```python
  async with async_session_factory() as session:
      for type_name, values in [
          ('hiringplantype', ['trial', 'short', 'medium', 'long']),
          ('hiringstatus', ['pending', 'under_review', 'approved', ...]),
      ]:
...
| Seed data loaded | `SELECT count(*) FROM vehicles` | count > 0 |
...
sleep 20
docker ps --filter "name=mvp_output-api"
```
...
This loop ran 15+ times during the Garage Service build.
...
- `[references/gloryev-seed-fk-cascade.md](references/gloryev-seed-fk-cascade.md)` — Seed data: FK cascade deletion order
- `[references/gloryev-seed-injection.md](references/gloryev-seed-injection.md)` — Seed data injection workflow: SQL vs Python seed scripts, docker cp/exec patterns, idempotent ON CONFLICT inserts, UnboundLocalError from shadowed imports
...
- `[references/gloryev-deployment-audit.md](references/gloryev-deployment-audit.md)` — Deployment findings (trailing-slash, DB name mismatch, orphan containers)
...
- `[references/gloryev-schema-drift-fix.md](references/gloryev-schema-drift-fix.md)` — Schema drift diagnosis: UndefinedColumnError, model↔table mismatch, migration sync
...
- `[references/car-hiring-feature.md](references/car-hiring-feature.md)` — Car hiring feature: DB table creation via direct SQL, UUID format requirement, progress stepper UX, plan selection workflow
- `[references/vehicle-image-loading.md](references/vehicle-image-loading.md)` — Vehicle image loading: host vs container filesystem, Wikimedia bot detection, SVG placeholders, Wikimedia API workflow (`ioprop=url`)
...
- `[references/byd-chinese-market-models.md](references/byd-chinese-market-models.md)` — 18 Chinese-market BYD models without images: Dazzle, Dolphin Mini, Han, Tang, etc.
- `[references/vehicle-image-naming-convention.md](references/vehicle-image-naming-convention.md)` — Vehicle image naming: `brand-model-{variant}-{color}` pattern, PIL color extraction, seed-to-image reconciliation, Google Drive copy via `shutil.copy()`
...
- `[references/garage-page-deployment-debug.md](references/garage-page-deployment-debug.md)` — Garage page: CSS staleness, HTML-CSS class mismatch, vehicle_service.py syntax fixes
- `[references/otel-instrumentation-pattern.md](references/otel-instrumentation-pattern.md)` — OpenTelemetry: graceful startup (no crash when OTel misconfigured), lazy imports, no-op span(), admin re-export pattern, Prometheus histogram label pitfall, automated indentation wrapping anti-pattern
...
- **Fix — write script to host, run via terminal:**
  ...
  3. The script pipes SQL to Docker via `subprocess.run(["docker", "compose", "-f", "docker-compose.uat.yml", "exec", "-T", "postgres", "psql", "-U", "gloryev", "-d", "gloryev_uat"], input=sql, capture_output=True, text=True)`
...
- **Verify after insert:** `docker compose exec -T postgres psql -U gloryev -d gloryev_uat -t -c "SELECT count(*) FROM users;"`
